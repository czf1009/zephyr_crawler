...
class Jd(object):
    ...
    def get_items(self):
        if self.cur.execute('''
            SELECT id,ware_id, DATE_FORMAT(`date`, '%Y-%m-%d') as date_day 
            FROM app_test.jd 
            order by ware_id,date;'''
            ):
            return self.cur.fetchall()
        else:
            print '\n\ninsert data ERROR!!!'
    ...
    def del_items(self,ids):
        if self.cur.executemany('delete from jd where id = %s;', ids):
            self.conn.commit()
            print "\nDelete jd complite.\n"
        else:
            print "Error happend when delete jd!!\n"
